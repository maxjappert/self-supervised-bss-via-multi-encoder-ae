...
from functions import TwoSourcesDataset
...
model_name = 'toy'
hps = json.load(open(f'hyperparameters/{model_name}.json'))
dataset_name = 'toy_dataset' if model_name.__contains__('toy') else 'musdb_18_prior'
image_h = 64
image_w = 64

num_samples_per_combo = 32
...
for vae_idx1, vae1 in enumerate(vaes):
    for vae_idx2, vae2 in enumerate(vaes):
        dataloader1 = DataLoader(test_datasets[vae_idx1], batch_size=num_samples_per_combo, shuffle=False, num_workers=4)
        dataloader2 = DataLoader(test_datasets[vae_idx2], batch_size=num_samples_per_combo, shuffle=False, num_workers=4)
        ...
        for i in range(num_samples_per_combo):
            print(i)
            source1, source2 = separate(batch_combined[i], hps, name=model_name, sigma_end=0.5, finetuned=True, visualise=False, verbose=False, k=2, constraint_term_weight=-18, stem_indices=[vae_idx1, vae_idx2])
