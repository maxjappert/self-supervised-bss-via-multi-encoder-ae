...
from sigma_experiment import num_samples

set_seed(42)

def evaluate_gradient_weight_video(name, num_samples=30, name_video_model='video_model_raft_resnet'):
    ...
    for weight_index, gradient_weight in enumerate(weights):
        ...
        np.save('results/gradient_video_weights_evaluated_0.npy', weights)
        np.save(f'results/gradient_video_weights_0.npy', basis)

def evaluate_gradient_weight(name, num_samples=128):
    print(f'Evaluating gradient weight for {name}\n')
    ...
    for weight_index, gradient_weight in enumerate(weights):
        for i in range(num_samples):
            ...
            sample = [test_datasets[stem_idx][random.randint(0, len(test_datasets[stem_idx])-1)]['spectrogram'] for stem_idx in stem_indices]

            # print(f'[{timestamp_str}]  Processing {i + 1}/{num_samples}')
            # to avoid, when the same stem is selected, the same sample
            gt_xs = torch.stack(sample, dim=0).to(device)

            gt_m = torch.sum(gt_xs, dim=0).to(device)

            # separate using basis
            separated = separate(gt_m, hps_vae, name=name, stem_indices=stem_indices, finetuned=False,
                                       alpha=1, visualise=False, verbose=False, gradient_weight=gradient_weight)
            separated = [x_i.detach().cpu().view(-1) for x_i in separated]

            sdr, isr, sir, sar, perm = mir_eval.separation.bss_eval_images(np.array([x.squeeze().detach().cpu().view(-1) for x in gt_xs]), separated, compute_permutation=True)
            # print(f'One metric took {time2 - time1} seconds')
        ...
        now = datetime.now()
        timestamp_str = now.strftime("%Y-%m-%d %H:%M:%S")
        print(f'[{timestamp_str}]  Weight: {gradient_weight}')
...
evaluate_gradient_weight('toy', num_samples=1)